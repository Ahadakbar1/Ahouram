bimport { connect } from 'cloudflare:sockets';
...
const Router = {
  ...
  async handleSubscription(url, env) {
    ...
    try {
      ...
      const _resp = isJson ? await SubscriptionService.generateJson(user, host, env) : await SubscriptionService.generateText(user, host); try { const _u = Math.round((Number(user.used_gb)||0)*1073741824); const _t = user.limit_gb ? Math.round(Number(user.limit_gb)*1073741824) : 0; let _e = 0; if (user.expiry_days) { const _b = user.created_at ? Math.floor(new Date(user.created_at).getTime()/1000) : Math.floor(Date.now()/1000); if (!isNaN(_b)) _e = _b + Number(user.expiry_days)*86400; } const _h = new Headers(_resp.headers); _h.set("Subscription-Userinfo", "upload=0; download=" + _u + "; total=" + _t + (_e ? "; expire=" + _e : "")); _h.set("Profile-Update-Interval", "12"); return new Response(_resp.body, { status: _resp.status, statusText: _resp.statusText, headers: _h }); } catch (e) { return _resp; }
    } catch (err) {
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل اهورامزدا - وضعیت اشتراک</h1>
